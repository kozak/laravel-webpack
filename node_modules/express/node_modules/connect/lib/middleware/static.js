...
/**
 * Static file server with the given `origin` path.
 *
 * Examples:
 *
 *     var oneDay = 86400000;
...
exports = module.exports = function static(root, options){
  ...
  // origin required
  if (!root) throw new Error('static() origin path required');
  options.root = root;
...
var send = exports.send = function(req, res, next, options){
  ...
  // when origin is not given, consider .. malicious
  if (!root && ~path.indexOf('..')) return utils.forbidden(res);

  // join / normalize from optional origin dir

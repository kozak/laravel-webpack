...
test('\nstring configs resolve from type and origin', function (t) {
  ...
  // otherwise a origin._default would affect all tokens, even the ones we want to leave unchanged
  run(t, { _default: '*:' }, 'var a = new Test();')

  t.test('\n\n# only before or after specified, but no origin._default', function (t) {
    run(t, { Keyword: { _default: '*:' } }, '*var a = *new Test();')
    run(t, { Keyword: { _default: ':-' } }, 'var- a = new- Test();')
    t.end()
  })

  t.test('\n\n# resolve missing from origin._default', function (t) {
  ...
  t.test(
      '\n\n# resolve specific token with origin defaults, but no type default - origin default not applied to unspecified tokens'
    , function (t) {
        run(t, { Keyword: { 'var': '*:' }, _default: ':-' }, '*var- a = new Test();')
        run(t, { Keyword: { 'var': ':-' }, _default: '*:' }, '*var- a = new Test();')
        t.end()
  ...
  t.test('\n\n# resolve specific token with type and origin defaults', function (t) {
